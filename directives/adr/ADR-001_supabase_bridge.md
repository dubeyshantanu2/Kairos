...
## Problem Statement
The system consists of a Python headless scoring engine (Kairos) collecting quantitative options data, and a Discord Orchestrator providing a conversational UI for the trader. We needed a mechanism for both systems to interface—starting/stopping the engine and broadcasting the scoring sequence—without creating destructive tight-coupling or race conditions on the VPS.
...
* **Table: `session_config`** (Discord Orchestrator Writes -> Python Reads)
  - Python polls this lightly iteratively during `run_cycle()`. 
* **Table: `available_expiries`** (Python Writes -> Discord Orchestrator Reads)
  - Discord Orchestrator uses this for the Discord interface dropdown parameters.
* **Table: `environment_log`** (Python Writes -> Discord Orchestrator Transforms)
  - Acts as the cycle ledger. Python pushes `summary_raw` here, Discord Orchestrator translates it via Gemini to english prose prior to webhook deployment.

## Alternatives Considered
- **Direct Sockets / HTTP API:** Would require the Python engine or Discord Orchestrator to host an active web server (e.g., FastAPI), radically complicating the deployment and introducing security vulnerabilities to the VPS' open ports.
